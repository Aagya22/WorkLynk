...
export const HrEmployeeList: React.FC = () => {
  ...
  const fetchUsersList = async () => {
    try {
      const response = await api.get('/api/admin/users?limit=50');
      if (response.data?.users) {
        // Filter out admin users or display them all (HR manager manages employees, but directory displays all)
  ...
  return (
    <DashboardLayout>
      <div className="space-y-8">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-100">Employee Directory</h1>
          <p className="text-sm text-slate-400 font-medium">
            Manage employee HR files, generate monthly payslips, and process GDPR portability requests.
          </p>
        </div>
        ...
        >
          <form onSubmit={handleGdprExportSubmit} className="space-y-4">
            ...
            <div className="bg-slate-950/40 p-4 rounded-xl border border-slate-900/50 space-y-2 text-xs text-slate-400 leading-normal">
              <h4 className="font-bold text-slate-300 uppercase tracking-wider">Third-Party Data Export Compliance:</h4>
              <p>
        ...
        >
          <div className="space-y-5 text-center py-2">
            ...
            <div className="space-y-2">
              <h3 className="text-base font-bold text-slate-200">Archive Exported: {zipFileName}</h3>
