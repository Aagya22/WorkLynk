...
export const Sidebar: React.FC<SidebarProps> = ({ user, logout, mobileOpen, setMobileOpen }) => {
  ...
  // A single nav row — icon + label, with the one accent for the active state.
  const NavRow: React.FC<{ item: NavEntry; showLabel: boolean; onNavigate?: () => void }> = ({ item, showLabel, onNavigate }) => {
    ...
    return (
      <Link
        ...
        onClick={onNavigate}
        className={`group relative flex h-10 items-center rounded-xl border transition-all duration-200 ${showLabel ? 'px-3' : 'justify-center px-0'} ${
          active
            ? 'sidebar-active border-accent-500/10 bg-accent-500/10 font-semibold text-strong shadow-glow'
            : 'border-transparent text-slate-400 hover:bg-white/5 hover:text-white'
        }`}
      >
        <span className={`flex-shrink-0 ${active ? 'text-accent-500' : 'text-slate-500 transition-colors group-hover:text-slate-200'}`}>
          {item.icon}
        </span>
  ...
  // Fixed bottom account/utility zone.
  const AccountZone: React.FC<{ showLabel: boolean; onNavigate?: () => void }> = ({ showLabel, onNavigate }) => (
    <div className="flex-shrink-0 space-y-1 border-t border-white/[0.06] bg-white/[0.02] pb-5 pt-3">
      ...
      <button
        ...
        title="Sign Out"
        className={`group relative flex h-10 w-full items-center rounded-xl text-slate-400 transition-all duration-200 hover:bg-red-500/15 hover:text-red-200 focus:outline-none ${
  ...
  return (
    <>
      {/* Rail spacer keeps content clear of the fixed sidebar */}
      <div className="hidden w-[60px] flex-shrink-0 md:block" />

      {/* Desktop sidebar — hover to expand */}
      <aside
        onMouseEnter={() => setExpanded(true)}
        onMouseLeave={() => setExpanded(false)}
        className={`app-sidebar ${expanded ? 'sidebar-expanded' : ''} fixed left-0 top-0 z-40 hidden h-screen flex-col overflow-hidden border-r border-[#1B2234] bg-[#070B18] backdrop-blur-xl transition-all duration-300 md:flex ${
          expanded ? 'w-[232px] px-3.5 shadow-2xl' : 'w-[60px] px-2'
        }`}
      >
        {/* Logo zone — brand mark, not a nav item */}
